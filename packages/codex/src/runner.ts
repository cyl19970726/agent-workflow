...
export type CodexInstalledVersionsOptions = {
  /** Optional workspace root checked before normal SDK/module resolution. */
  packageRoot?: string;
...
function installedPackageVersion(packageName: string, packageRoot?: string): string {
  try {
    if (packageRoot) {
      const workspaceManifest = path.join(packageRoot, "node_modules", ...packageName.split("/"), "package.json");
...
function installedCliVersion(binary = "codex"): string {
  try {
...
export function codexInstalledVersions(options: CodexInstalledVersionsOptions = {}): { sdkVersion: string; codexRuntimeVersion: string } {
  return {
    sdkVersion: installedPackageVersion("@openai/codex-sdk", options.packageRoot),
    codexRuntimeVersion: installedCliVersion(options.cliBinary)
  };
}
