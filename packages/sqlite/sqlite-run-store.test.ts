...
describe("SQLite workflow ledger", () => {
  it("survives adapter recreation, scopes replay and preserves attempts", async () => {
    ...
    const run = await store.createRun({ workflowId: "post", workflowRevision: "1", inputFingerprint: "input", state: "running", metadata: { tenant: "example" } });
    ...
    expect(await reopened.listRuns({ metadata: { tenant: "example" } })).toHaveLength(1);
