...
Domain workflows and production dispatch live in the consuming application. They own candidate versions, repair policies, independent review, business validation, and project skills.

## Execution and recovery

- `RunStore` owns run, step, attempt, artifact and event persistence. SQLite implementation is in `@signal-room/workflow-sqlite`; `MemoryRunStore` is for tests and transient use.
...
- A durable `mapSettled` finishes its current dispatch work before suspending. The dispatcher/scheduler is responsible for actual worker concurrency. The host supplies its own queue and concurrency policy.
...
- Runtime cancellation is an `AbortSignal`. Durable cancellation, descendant propagation and cross-process polling are adapter responsibilities. The host remains the only scheduling authority.
